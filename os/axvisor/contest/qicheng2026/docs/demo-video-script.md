...
总结一下，我们完成了 AxVisor 上 Linux/RTOS 双 Guest 部署，RTOS Guest 使用 Zephyr e1000 IP 网络，Linux 和 RTOS 之间用 QCZ1 可靠 UDP 协议通信，Linux 侧 AI 推理驱动 RTOS 控制状态更新，并提供了原生 RTOS 基线、双 Guest 长样本实时性数据、2-worker 三轮稳定性复核、复现脚本和核心 patch 拆分说明。后续提交会先提交 contest 目录，再把 VM config、vTimer 和 GIC 中断路径修正拆成独立核心 patch 审查。
