...
def request_status(sock: socket.socket, peer: tuple[str, int], seq: int, timeout: float) -> bool:
    ...
    print(
        f"status result=STATUS last_seq={last_seq} status={status} "
...
def main() -> int:
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as sock:
        ...
        status_ok = request_status(sock, peer, args.count + 1000, args.timeout)
