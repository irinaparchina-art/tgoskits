...
| Reproducibility | PASS | `docs/reproduce.md`, one-command integrated runner, analyzer scripts, SHA256 evidence records |
...
| Describe vCPU/physical CPU binding, memory, devices, interrupts and boot args | Guest and topology docs describe Linux `2` vCPU setup, `br-qc-dual`, virtio-net, Zephyr e1000, interrupt route and `noirqdebug` run. | See `docs/design.md`, `docs/network-topology.md` and `docs/realtime-evaluation.md`. |
...
| Requirement | Current evidence | Reviewer note |
| --- | --- | --- |
| Use IP protocol stack as the main data channel | The main channel is IPv4/UDP over `br-qc-dual`, with Linux virtio-net and Zephyr e1000. | `docs/network-topology.md` states that shared memory, HyperCall, MMIO and vsock are not the primary data path. |
...
| Requirement | Current evidence | Reviewer note |
| --- | --- | --- |
| Deploy neural-network inference in Linux/Starry-style guest | Linux-side AI control demo uses a deterministic small MLP implemented in the static guest demo/client path. | The model is intentionally lightweight and reproducible for the contest demo. |
...
| Demonstrate closed loop | Integrated run includes AI input, inference, QCZ1 network transfer, RTOS control output and state reply. | One-command run requires `QC_AI_CONTROL_RESULT=PASS`. |
...
| Landing feasibility 25% | Uses one-command QEMU reproduction, explicit topology, deterministic model and clear first-stage/core-patch separation. |
